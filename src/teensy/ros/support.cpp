#include "ros/support.h"

#include "ros/check_err.h"

#include <rclc/executor.h>
#include <rclc/init.h>
#include <rclc/node.h>
#include <rclc/timer.h>

namespace ros {

Support::Support() : allocator_(rcl_get_default_allocator()) {
  ROS_CHECK_ERR(rclc_support_init(&impl_, 0, nullptr, &allocator_));
}

Support::~Support() { ROS_CHECK_ERR(rclc_support_fini(&impl_)); }

void Support::init(rcl_node_t &node, const char *name) {
  ROS_CHECK_ERR(rclc_node_init_default(&node, name, "", &impl_));
}

void Support::init(rcl_timer_t &timer, int64_t timeout_ns, rcl_timer_callback_t callback) {
  ROS_CHECK_ERR(rclc_timer_init_default(&timer, &impl_, timeout_ns, callback));
}

void Support::init(rclc_executor_t &executor, size_t num_handles) {
  ROS_CHECK_ERR(rclc_executor_init(&executor, &impl_.context, num_handles, &allocator_));
}

void Support::finalize(rcl_timer_t &timer) { ROS_CHECK_ERR(rcl_timer_fini(&timer)); }

void Support::finalize(rclc_executor_t &executor) { ROS_CHECK_ERR(rclc_executor_fini(&executor)); }
} // namespace ros